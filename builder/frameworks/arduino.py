...
# `cores/arduino/api` is a git symlink to `ArduinoCore-API/api`. Git for Windows
# only materializes symlinks when core.symlinks=true, so on a stock Windows
# checkout it is a text file holding the target path. SCons then rejects it as a
# CPPPATH entry, and the cores/ glob below misses the Wiring API sources behind
# it. Resolve to the real directory; a no-op where the symlink survived.
ARDUINO_API_DIR = os.path.join(FRAMEWORK_DIR, "cores", "arduino", "api")
ARDUINO_API_IS_SYMLINK = os.path.isdir(ARDUINO_API_DIR)
if not ARDUINO_API_IS_SYMLINK:
    ARDUINO_API_DIR = os.path.join(FRAMEWORK_DIR, "ArduinoCore-API", "api")
assert os.path.isdir(ARDUINO_API_DIR)
...
# NTFS is case-insensitive too, so the same `String.h` case-shadow breaks
# libstdc++'s <cstring> under MinGW-w64. The Darwin fix above doesn't port (it
# hardcodes libc++'s _LIBCPP_STRING_H and an xcrun lookup), but the shape does:
# materialize a `string.h` on a novel path that absolute-includes the real
# header. #include_next would resume from the shim dir and hit `String.h` again,
# and -I<ucrt-include> is ignored because GCC skips -I entries naming a standard
# system directory. Ask the compiler where <string.h> actually resolves.
WINDOWS_STRING_SHIM_DIR = None
if host_platform.system() == "Windows":
    try:
        cc = env.subst("$CC") or "gcc"
        proc = subprocess.run(
            [cc, "-E", "-xc", "-"],
        ...
        real_string_h = None
        # Linemarkers look like: # 1 "C:/msys64/ucrt64/include/string.h" 1 3
        for line in proc.stdout.splitlines():
            ...
            if len(parts) < 2:
                continue
            candidate = parts[1]
            if os.path.basename(candidate).lower() == "string.h" and os.path.isfile(
                candidate
            ):
                real_string_h = candidate
                break
        if real_string_h:
            shim_dir = os.path.join(
                env.subst("$PROJECT_BUILD_DIR"),
                "framework-portduino-windows-shim",
            )
            os.makedirs(shim_dir, exist_ok=True)
            shim_path = os.path.join(shim_dir, "string.h")
            shim_content = (
                "// Auto-generated by platform-native/builder/frameworks/"
                "arduino.py.\n"
                "// Routes <string.h> away from ArduinoCore-API's "
                "case-clashing String.h.\n"
                "#pragma once\n"
                '#include "%s"\n' % real_string_h.replace("\\", "/")
            )
            existing = ""
            if os.path.isfile(shim_path):
                with open(shim_path, "r") as f:
            ...
            if existing != shim_content:
                with open(shim_path, "w") as f:
                    f.write(shim_content)
            if os.path.isfile(shim_path):
                WINDOWS_STRING_SHIM_DIR = shim_dir
...
if WINDOWS_STRING_SHIM_DIR:
    env.Prepend(CPPPATH=[WINDOWS_STRING_SHIM_DIR])
...
# The glob above never reached the Wiring API sources when the symlink didn't
# survive checkout; build them from the real directory instead.
if not ARDUINO_API_IS_SYMLINK:
    libs.append(env.BuildLibrary(
